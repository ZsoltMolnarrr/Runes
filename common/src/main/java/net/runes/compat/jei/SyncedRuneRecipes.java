...
public final class SyncedRuneRecipes {
    private static volatile Supplier<List<RecipeEntry<RuneCraftingRecipe>>> source = List::of;

    private SyncedRuneRecipes() {}

    /** Installed once per platform, from the client initializer. */
    public static void setSource(Supplier<List<RecipeEntry<RuneCraftingRecipe>>> supplier) {
        source = supplier;
    }

    /** The altar recipes the server has synced, or an empty list when not connected / not yet synced. */
    public static List<RecipeEntry<RuneCraftingRecipe>> get() {
        return source.get();
    }
}
